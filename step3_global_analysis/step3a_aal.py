"""
Step 3a threshold-sweep diagnostic for AAL, all three sign strategies,
group-blind.

In: cached matrices via config.atlas_dir("aal", "step2_pipeline"),
    config.GROUP_CSV.
Out: config.atlas_dir("aal", "step3a_sweep", cross_strategy=True)/
...
run_threshold_sweep(
    paths_cfg = {
        "mat_dir"     : config.atlas_dir("aal", "step2_pipeline") / "comet_matrices",
        "csv_path"    : config.GROUP_CSV,
        "out_dir"     : config.atlas_dir("aal", "step3a_sweep", cross_strategy=True),
